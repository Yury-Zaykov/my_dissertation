...
function FindHU_up(cnt_HU, i)
    ...
    for m = 1:1:length(SortHU(1, :))
        ...
        % нужно перевести ГА и переводим его в эту след. зону      
        v = 3;
        for c = 1:1:maxZone
            if (SortHU(2, 1) == c)
                temp_CtlCheckZoneHU = CtlCheckZoneHU(1, NumHU);
                CtlCheckZoneHU(1, NumHU) = c + 1;
                ctlHU_P(1, NumHU) = ZoneUnWork(v , NumHU) * 1.015;
                break;
            end
            v = v + 2;
        end
        ...
        % В зависимости от зоны работы ГА, рассчитываем мощность ГА на разгрузку
        for k = 1:1:cnt_HU
            v = 1;
            % Проверяем для всех возможных зон для данного ГА
            for c = 1:1:maxZone
                if (CtlCheckZoneHU(1, k) == c)
                    toUnload(1, k) = ctlHU_P_temp(1, k) - ZoneUnWork(v, k);
                    break;
                end
                v = v + 2;
            end
